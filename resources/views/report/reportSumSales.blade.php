<x-navbar :title="$title" activeGroup="statistic" activeName="sumSales"></x-navbar>
                    ...
                    <div class="dropdown-menu" id="dropdown-menu"
                        ...
                        <div class="scrollbar">
                            ...
                            <button class="dropdown-item btndropdown text-13-black" id="btn-sales"
                                data-button="sales" type="button">CTV bán hàng</button>
                            ...
                            <button class="dropdown-item btndropdown text-13-black" id="btn-slban"
                                data-button="slban" type="button">SL bán</button>
                        </div>
                    </div>
                    <!-- Input fields for filtering -->
                    <x-filter-date-time name="date" title="Ngày" />
                    <x-filter-text name="maphieu" title="Số chứng từ" />
                    <x-filter-checkbox :dataa='$users' name="sales" title="CTV bán hàng"
                        namedisplay="name" />
            ...
            <div class="d-flex content__heading--right">
                <div class="row m-0">
                    ...
                    <button class="mx-1 d-flex align-items-center btn-primary rounded"
                        ...
                        trang</button>
                    <form id="exportForm" action="{{ route('exportSalesReport') }}" method="GET"
                        style="display: none;">
                        @csrf
                                            ...
                                            @foreach ($groupGuests as $value)
                                                ...
                                                @foreach ($guest as $item)
                                                    @if ($item->group_id == $value->id)
                                                        <tr>
                                                            <td class="border-bottom bold"></td>
                                                            <td colspan="9" class="border-bottom bold">Khách
                                                                hàng:
                                                                {{ $item->guest_name_display }}</td>
                                                        </tr>
                                                        @php
                                                        ...
                                                        <!-- Hiển thị tổng cộng số lượng bán cho khách hàng này -->
                                                        <tr>
                                                            <td colspan="5" class="text-right border">Tổng cộng
                                                                số
                                                                lượng bán:</td>
                                                            <td class="border">
...
<script>
    $(document).on('click', '.btn-submit', function(e) {
        ...
        var formData = {
            ...
            sales: getStatusData(this, 'sales'), // CTV bán hàng
            code: getData('#code', this), // Mã hàng
            name: getData('#name', this), // Tên hàng
            dvt: getData('#dvt', this), // ĐVT
            slban: retrieveComparisonData(this, 'slban'), // SL bán
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                updateFilters(data, filters, '.result-filter-product', '.-sell',
