                        ...
                        <div class="row mr-0">
                            <div class="col-md-5 d-flex align-items-center">
                                ...
                                <div class="dropdown mx-2 filter-all">
                                    ...
                                    <div class="dropdown-menu" id="dropdown-menu" aria-labelledby="dropdownMenuButton"
                                        ...
                                        <div class="scrollbar">
                                            <button class="dropdown-item btndropdown text-13-black" id="btn-provides"
                                                data-button="provides" type="button">Tên hiển thị
                                            </button>
                                            ...
                                            <button class="dropdown-item btndropdown text-13-black"
                                                id="btn-provide_code" data-button="provide_code" type="button">Mã số
                                                thuế
                                            </button>
                                    ...
                                    <x-filter-text name="provide_code" title="Mã số thuế" />
                                    <x-filter-checkbox :dataa='$provides' name="provides" title="Tên hiển thị"
                                        ...
                                        @foreach ($groups as $value)
                                            ...
                                            @if ($value->getAllProvides)
                                                @php
                                                    $total = 0;
                                                @endphp
                                                @foreach ($value->getAllProvides as $item)
                                                    <tr class="position-relative provide-info height-52">
                                                        <input type="hidden" name="id-provide" class="id-provide"
                                                            id="id-provide" value="{{ $item->id }}">
                                                        <td class="border-top-0 border-bottom">
    ...
    $(document).on('click', '.btn-submit', function(e) {
        ...
        if ($(this).data('button-name') === 'provides') {
            $('.ks-cboxtags-provides input[type="checkbox"]').each(function() {
                ...
                if ($(this).is(':checked') && provides.indexOf(value) === -1) {
                    provides.push(value);
                } else if (!$(this).is(':checked')) {
                    const index = provides.indexOf(value);
                    if (index !== -1) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.result-filter-provide').empty();
                if (data.filters.length > 0) {
                    $('.result-filter-provide').addClass('has-filters');
                } else {
                    $('.result-filter-provide').removeClass('has-filters');
                }
                ...
                data.filters.forEach(function(item) {
                    ...
                    $('.result-filter-provide').append(itemFilter);
                });
