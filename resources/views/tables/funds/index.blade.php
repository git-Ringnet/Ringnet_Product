            ...
            <div class="d-flex align-items-center pl-3">
                ...
                <div class="dropdown mx-2 filter-all">
                    ...
                    <div class="dropdown-menu" id="dropdown-menu"
                        ...
                        <div class="scrollbar">
                            <button class="dropdown-item btndropdown text-13-black" id="btn-tenquy"
                                data-button="tenquy" type="button">Tên quỹ
                            </button>
                            <button class="dropdown-item btndropdown text-13-black" id="btn-tienquy"
                                data-button="tienquy" type="button">Tiền quỹ
                            </button>
                            <button class="dropdown-item btndropdown text-13-black" id="btn-date"
                                data-button="date" type="button">Ngày bắt
                                đầu
                            </button>
                        </div>
                        <!-- Filter input fields corresponding to the new filters -->
                        <x-filter-text name="tenquy" title="Tên quỹ" />
                        <x-filter-compare name="tienquy" button="tienquy" title="Tiền quỹ" />
                        <x-filter-date-time name="date" title="Ngày bắt đầu" />
                    </div>
                </div>
            </div>
                        ...
                        <div class="outer2 table-responsive text-nowrap">
                            <table id="example2" class="table table-hover bg-white rounded">
                                <thead class="border-custom">
                                    <tr class="height-30">
                                        <th class="py-0 border" scope="col" style="">
                                        ...
                                        <th class="py-0 border" scope="col" style="">
                                            <span class="d-flex justify-content-start">
                                                <a href="#" class="sort-link btn-submit"
                                                    ...
                                                    <button class="btn-sort" type="submit">
                                                        <span class="text-14">Tên quỹ</span>
                                                    </button>
                                                </a>
                                                <div class="icon"></div>
                                            </span>
                                        ...
                                        <th class="py-0 border" scope="col" style="">
                                            <span class="d-flex justify-content-start">
                                                <a href="#" class="sort-link btn-submit"
                                                    data-sort-by="group_type" data-sort-type="DESC">
                                                    <button class="btn-sort" type="submit">
                                                        <span class="text-14">Tiền quỹ</span>
                                                    </button>
                                                </a>
                                                <div class="icon"></div>
                                            </span>
                                        ...
                                        <th class="py-0 border" scope="col" style="">
                                            <span class="d-flex justify-content-start">
                                                <a href="#" class="sort-link btn-submit"
                                                    data-sort-by="group_type" data-sort-type="DESC">
                                                    <button class="btn-sort" type="submit">
                                                        <span class="text-14">Ngày bắt đầu</span>
                                                    </button>
                                                </a>
                                                <div class="icon"></div>
                                            </span>
                                ...
                                <tbody class="tbody-fund">
                                    ...
                                    @foreach ($funds as $fund)
                                        <tr class="position-relative fund-info height-30">
                                            <input type="hidden" name="id-fund" class="id-fund" id="id-fund"
                                                value="{{ $fund->id }}">
                                            <td class="text-13-black border-bottom border py-0">
                                                {{ $stt++ }}
                                            </td>
                                            <td class="text-13-black border-bottom border py-0">{{ $fund->name }}
                                            </td>
                                            <td class="text-13-black border-bottom border py-0">
                                                ...
                                                <div class="d-flex w-100">
                                                    <a href="{{ route('funds.show', $fund->id) }}">
                                                        <div class="m-0 px-2 py-1 mx-2 rounded">
                                                            <i class="fa-regular fa-eye text-secondary py-1"
                                                                aria-hidden="true"></i>
                                                        </div>
                                                    </a>
                                                    ...
                                                    <a href="#">
                                                        <div class="rounded">
                                                            <form
                                                                ...
                                                                action="{{ route('funds.destroy', $fund->id) }}"
...
<script type="text/javascript">
    ...
    $(document).on('click', '.btn-submit', function(e) {
        ...
        var formData = {
            ...
            fundName: getData('#tenquy', this), // Lấy dữ liệu từ trường Tên quỹ
            fundAmount: retrieveComparisonData(this, "tienquy"), // So sánh dữ liệu cho Tiền quỹ
            date: retrieveDateData(this, "date"), // Dữ liệu cho Ngày bắt đầu
        ...
        $.ajax({
            type: 'get',
            url: "{{ route('searchFunds') }}",
            data: formData,
            success: function(data) {
                updateFilters(data, filters, '.result-filter-fund', '.tbody-fund',
                    '.fund-info',
                    '.id-fund', $(this).data('button'));
                updateValuesByClass(sumValuesByClassAndId('fund_debt', 'id', 'tr'),
                    'fund_debt_total');
                updateValuesByClass(countClassOccurrencesById('fund_debt', 'id', 'tr'),
                    'total_fund');
            }
        });
        $.ajaxSetup({
