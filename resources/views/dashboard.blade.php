<x-app-layout>
    ...
    {{-- <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight title-data">
                Quản lý Workspaces
            </h2>
        </x-slot> --}}
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div
                class="info w-100 d-flex justify-content-center"style="width: 100%;justify-content: center;display: grid;">
                <h1>Cập nhật thông tin doanh nghiệp</h1>
                <div class="border" style="border-radius: 4px; width: 380px;">
                    <form action="#" method="post">
                        @csrf
                        <div class="p-2 form-control">
                            <label for=""
                                style="font-family: Inter;font-size: 14px;font-weight: 500;text-align: left;">Tên
                                doanh nghiệp</label>
                            <br>
                            ...
                            <br>
                            <label for=""
                                style="font-family: Inter;font-size: 14px;font-weight: 500;text-align: left;">Số
                                điện thoại*</label>
                            <br>
            ...
            <div
                class="workspace w-100 d-flex justify-content-center"style="width: 100%;justify-content: center;display: none;">
                @if (!$issetworkspace)
                    <h1>Tạo workspace mới</h1>
                    <form action="{{ route('createWorkspace') }}" method="POST" style="width: 400px">
                        @csrf
                        <label for=""
                            style="font-family: Inter;font-size: 14px;font-weight: 500;text-align: left;">Workspace
                            name*</label>
                        <br>
    ...
    <div class="d-none" style="display: none">
        ...
        <form action="{{ route('importDatabase') }}" enctype="multipart/form-data" method="POST"
            id="restore_data1" class="btn btn-outline-primary d-flex align-items-center h-100 custom-btn">
            @csrf
            <label class="btn-file mb-0 wf-500" style="color:#0095F6; font-weight : 500">
                ...
                Khôi phục<input type="file" style="display: none;" id="file_restore1" accept=".sql"
                    name="file">
            </label>
        </form>
    </div>
...
<script>
    ...
    $(document).on('change', '#file_restore1', function(e) {
        e.preventDefault();
        $('#restore_data1')[0].submit();
    })
