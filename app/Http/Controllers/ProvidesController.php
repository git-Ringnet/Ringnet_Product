...
class ProvidesController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($request->ajax()) {
            $output = '';
            $provides = $this->provides->ajax($data);
            if (!empty($request->input('idName'))) {
                $arrprovideName = $this->provides->getprovidebyName($data);
            }
            if (!empty($request->input('idCode'))) {
                $arrCodeName = $this->provides->getprovidebyCode($data);
            }
            if ($provides) {
                foreach ($provides as $key => $product) {
                    $url = route('provides.edit', $product->id);
                    $deleteUrl = route('provides.destroy', $product->id);
                    $output .= '<tr>
                    <td><input type="checkbox"></td>
                    <td>' . $product->provide_name_display . '</td>
                    <td>' . $product->provide_code . '</td>
                    <td>' . $product->provide_email . '</td>
                    <td>' . $product->provide_phone . '</td>
                    <td>' . number_format($product->provide_debt) . '</td>
                    <td>' . '<a href="' . $url . '">
