...
class Delivery extends Model
{
    ...
    public function acceptDelivery($data)
    {
        ...
        for ($i = 0; $i < count($data['product_name']); $i++) {
            ...
            $history_import = HistoryImport::where('product_id', isset($data['product_id'][$i]))->first();
            // Add lịch sử giao dịch
            $htrImport = HistoryImport::where('product_id', isset($data['product_id'][$i]))
                ->where('workspace_id', Auth::user()->current_workspace)
                ->orderBy('id', 'ASC')
                ->get();
            $qty = 0;
            ...
            if ($htrImport) {
                $check = false;
                foreach ($htrImport as $va) {
                    // Kiểm tra sản phẩm có SN không
                    $product = Products::where('id', $va->product_id)->first();
